...
void serve_static(int fd, char *filename, int filesize, char *method) { // 클라이언트 연결 식별자, 요청 받은 파일이름, 파일 크기
  ...
  char *srcp, filetype[MAXLINE], buf[MAXBUF]; // 메모리 매핑된 파일의 주소를 가리킬 포인터, MIME 타입을 저장할 배열, HTTP 응답 헤더를 구성하기 위한 버퍼
  ...
  if (!(strcasecmp(method, "HEAD") ==0))
  {
    ...
    Rio_writen(fd,srcp,filesize); // 주소 srcp에서 시작하는 filesize 바이트를 클라의 연결 식별자로 복사
    free(srcp); // 사용이 끝난 메모리 매핑을 해제
  }
}
